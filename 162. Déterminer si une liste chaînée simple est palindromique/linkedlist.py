...
class LinkedList:
    ...
    def inverse(self):
        ...
        if self.first is None:
            raise ValueError("On ne peut pas inverser une liste chaînée vide.")
        ...
        if self.first != self.last:
            ...
            #Boucle principale d'inversion de la liste chaînée
    ...
    def get_middle_node(self):
        """Renvoie le noeud central de la liste chaînée.
        
        Leetcode 876 de l'application leetcode Python"""

        if self.length() == 0:
            ValueError("This linked list is empty. Can't find the middle node.")
    ...
    def swap_pairs(self):
        """Inverse chaque pair de noeuds dans la liste chaînée"""

        if self.length() == 0:
            raise ValueError("Can't swap pairs in empty linked list")
        ...
        if self.length() > 1:
            ...
            while node and node.next:
                ...
                #Si on n'est pas encore à la fin de la liste chaînée
                if temp and temp.next:
                    bridge_pair = node.next.next.next

                #Si on est à la dernière paire d'une liste chaînée de taille impaire
                elif temp and not temp.next:
                    bridge_pair = temp

                #Si on est à la dernière paire d'une liste chaînée de taille paire
                else:
                    bridge_pair = temp
                    self.last = node

                node.next.next = node
                node.next = bridge_pair
                node = temp



    def is_cyclic(self):
        """Renvoie True si cette liste chaînée est cyclique. Renvoie False sinon.
        
        Leetcode 141 de l'application Leetcode Python"""

        fast, slow = self.first, self.first
        while fast and fast.next:
            slow = slow.next
            fast = fast.next.next
            if fast == slow:
    ...
    def first_node_in_cycle(self):
        """Renvoie le noeud qui commence le cycle si cette liste chaînée est cyclique.
        
        Leetcode 142 de l'application Android Leetcode Python"""
        ...
        #Si la liste chaînée n'est pas cyclique, on ne renvoie rien.
        return None



    def partition(self, x):
        """Partitionne la liste chaînée en deux. D'un côté toutes les valeurs inférieures à x.
        De l'autre, toutes les valeurs supérieures à x. Chaque noeud doit garder sa position
        relative par rapport aux autres.
        
        Leetcode 086 de l'application Android Leetcode Python."""

        lesser_first = lesser = Node(None)
        greater_first = greater = Node(None)
        ...
        greater.next = None
        lesser.next = greater_first.next

        #On instancie la liste partitionnée à renvoyer
        partitioned_linked_list = LinkedList()
        partitioned_linked_list.first = lesser_first.next
        partitioned_linked_list.last = greater 
        return partitioned_linked_list


    
    def is_palindrome(self):
        """Détermine si la liste chaînée est palindromique. Si elle l'est, la méthode renvoie True
        sinon elle renvoie False.
        
        Leetcode 234 de l'application Android Leetcode Python."""
